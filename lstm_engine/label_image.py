...
def get_labels(text_data, cwd):
    # Loads label file, strips off carriage return
    label_lines = [
        line.rstrip() for line in tf.io.gfile.GFile(cwd + "/content_labels.txt")
    ]
    # print(label_lines)
    # Unpersists graph from file
    with tf.io.gfile.GFile(cwd + "/content_graph.pb", "rb") as f:
        graph_def = tf.compat.v1.GraphDef()
        graph_def.ParseFromString(f.read())
        _ = tf.import_graph_def(graph_def, name="")

    # Feed the text_data as input to the graph and get first prediction
    ...
    # Get the top kind of hate speech after classification
    type_of_issue = label_lines[top_k[0]]
    return type_of_issue, labels_and_scores

# return title, statistics and summary
def wiki(type_of_issue,cwd):
